...
## 它可以做什么
只需对 AI 说：「**今日红单**」或「**怎么买**」
  → **智能避热逻辑** (自动下调顶级豪门权重，警惕热门陷阱) → 赛事全量抓取 → **多维情报采集** (H2H + 赔率走势 + 伤停核实) → **多重加权评分** (赔率走势为主 + 情报 + 历史) → 生成详版分析报告
...
### 2. 多维加权评分模型
不再只看赔率差距，新模型综合了三大维度（与 `SKILL.md` 一致）：
- **赔率走势 (50%)**：最高权重。水位变动与变盘方向。
- **实时情报 (30%)**：联网核实核心伤停与阵容。
- **历史底蕴 (20%)**：H2H 与近期战绩作辅助。
...
| **单篇微信复盘** | 链接拉 HTML + 自动下载 `#js_content` 配图，读图还原胜平负/让球与场次矛盾 | `SKILL.md` + `scripts/parse_wechat_article_html.py` + `scripts/fetch_wechat_article_images.py` |
...
- **查赔率**：`python3 scripts/fetch_match_data.py --match <id> --odds-only`（**不用于**「赔率截图模式」— 该模式下仅以用户截图为准）
